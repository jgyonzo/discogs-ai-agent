...
- [ ] T029 [US2] Create the curated questions data file at `frontend/src/data/curatedQuestions.ts` exporting `curatedQuestions: readonly CuratedQuestion[]` containing all 7 entries from [contracts/curated-questions.md](./contracts/curated-questions.md) §1 (Q1 Releases by decade, Q2 Techno over time, Q3 Vinyl vs CD, Q4 Top countries, Q5 Label diversity, Q6 House outliers, Q7 Works with most versions). Field-for-field match required; the contract is normative. Use `as const` on the array literal so TypeScript widens types tightly.
- [ ] T030 [US2] Create `frontend/src/components/SuggestedQuestions.tsx`: groups the curated questions by `category` and renders each as a card with title, description, and two buttons "Use" and "Run" per spec §22. Props: `onUse(query: string): void`, `onRun(query: string): void`. Layout: vertical stack of category groups, each group has a small heading; cards inside the group are a responsive grid (1 column on `sm`, 2 on `md`). Lucide icons: `<Search>` next to "Use", `<Play>` next to "Run". The groups render in the order categories first appear in the array (Trends → Styles → Formats → Geography → Labels → Advanced → Masters in the V1 set).
- [ ] T031 [US2] Wire `SuggestedQuestions` into `App.tsx`. Place it as a left sidebar (or above the ChatPanel on small screens — TBD by spec §10 layout). Handlers: `onUse(q) => setInputValue(q)` (lift QueryInput's text state to App, or use a ref-style approach — simplest: lift it); `onRun(q) => submit(q)` reusing the US1 submit thunk. The chat panel's empty-state from T021 should fade or hide when curated questions are present in the sidebar (so the user isn't double-prompted to start). Modify `App.tsx`'s grid layout: header full-width, then 3-column on `lg+` (sidebar 25% / chat 45% / result 30%), 2-column on `md` (sidebar collapses into a drawer or inline list above chat — V1 simplest: stack vertically on `md-`). Depends on T029, T030, and US1 wiring (T023).

### Tests for User Story 2

- [ ] T032 [P] [US2] Component test `SuggestedQuestions` at `frontend/tests/components/SuggestedQuestions.test.tsx`: renders all 7 questions with their titles visible; "Use" button click calls `onUse` with the exact `query` text; "Run" button click calls `onRun` with the exact `query` text; questions are grouped by category (assert via heading hierarchy or by `data-category` attribute); empty data array (theoretical) renders nothing without crashing.
- [ ] T033 [P] [US2] Spread-coverage test at `frontend/tests/integration/curated-questions-spread.test.ts` per [contracts/curated-questions.md](./contracts/curated-questions.md) §5: assert `curatedQuestions.length >= 5` (FR-005 floor); the union of all `demonstrates` arrays has size `>= 5` (the meaningful-spread interpretation); each entry has `title.length <= 40`, `description.length <= 100`, `category` ∈ the 7-value enum, `query.length > 0`. This test is the regression guard for the curated set — if a future contributor edits the data file in a way that breaks the spread, CI catches it.
