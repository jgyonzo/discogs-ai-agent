...
- [ ] T018 ⏳ OWNER-RUN (partial evidence 2026-07-06: tool run against the real snapshot produced a link that 303s to a `list=TLGG…` temp playlist; browser click-through, play-order check, and site-side save still pending) — Live replay (owner-run, quickstart): SC-001 — two-prompt journey, click a returned link, verify play order matches the listing and the site-side "Save playlist" works; SC-002 — audit every `video_ids` entry in the emitted links against the records' `media_links` output (zero unexplained ids, zero non-payload YouTube URLs in answers); record results as a spec addendum if any deviation surfaces (018 precedent)
